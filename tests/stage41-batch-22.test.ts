...
import {
  batch22MotifIds,
  stage41ExtractionAuditCases,
  stage41RealDreamAuditCases,
  stage41RepresentativeDreams,
} from "./fixtures/stage41";

describe("Stage41 batch-22", () => {
  it("registers all batch-22 motifs", () => {
    const motifIds = motifRegistry.motifs.map((motif) => motif.id);

    expect(motifIds).toEqual(expect.arrayContaining(batch22MotifIds));
    expect(batch22MotifIds).toHaveLength(10);
  });

  it("keeps registry warning gates stable after batch-22 addition", () => {
  ...
  it("keeps batch-22 relations classified outside general and stale buckets", () => {
    const records = buildRelationReviewRecords().filter(
      (record) =>
        batch22MotifIds.includes(record.sourceId as (typeof batch22MotifIds)[number]) ||
        batch22MotifIds.includes(record.targetId as (typeof batch22MotifIds)[number]),
    );
  ...
  it("keeps batch-22 copy non-deterministic (no 絶対/必ず/断言/確実に)", () => {
    const forbiddenPhrases = ["絶対", "必ず", "断言", "確実に"];

    for (const motifId of batch22MotifIds) {
  ...
  it.each(stage41ExtractionAuditCases)("extracts documented Stage41 audit case $id", (auditCase) => {
  ...
  it.each(stage41RealDreamAuditCases)("keeps Stage41 real-dream extraction audit comparable for $id", (auditCase) => {
    const motifIds = extractMotifs(auditCase.text).map((candidate) => candidate.motif.id);
  ...
  it.each(stage41RepresentativeDreams)("keeps Stage41 representative dream behavior for $id", (fixture) => {
