...
import {
  batch23MotifIds,
  stage43ExtractionAuditCases,
  stage43RealDreamAuditCases,
  stage43RepresentativeDreams,
} from "./fixtures/stage43";

describe("Stage43 batch-23", () => {
  it("registers all batch-23 motifs", () => {
    const motifIds = motifRegistry.motifs.map((motif) => motif.id);

    expect(motifIds).toEqual(expect.arrayContaining(batch23MotifIds));
    expect(batch23MotifIds).toHaveLength(10);
  });

  it("keeps registry warning gates stable after batch-23 addition", () => {
  ...
  it("keeps batch-23 relations classified outside general and stale buckets", () => {
    const records = buildRelationReviewRecords().filter(
      (record) =>
        batch23MotifIds.includes(record.sourceId as (typeof batch23MotifIds)[number]) ||
        batch23MotifIds.includes(record.targetId as (typeof batch23MotifIds)[number]),
    );
  ...
  it("keeps batch-23 copy non-deterministic (no 絶対/必ず/断言/確実に)", () => {
    const forbiddenPhrases = ["絶対", "必ず", "断言", "確実に"];

    for (const motifId of batch23MotifIds) {
  ...
  it.each(stage43ExtractionAuditCases)("extracts documented Stage43 audit case $id", (auditCase) => {
  ...
  it.each(stage43RealDreamAuditCases)("keeps Stage43 real-dream extraction audit comparable for $id", (auditCase) => {
    const motifIds = extractMotifs(auditCase.text).map((candidate) => candidate.motif.id);
  ...
  it.each(stage43RepresentativeDreams)("keeps Stage43 representative dream behavior for $id", (fixture) => {
