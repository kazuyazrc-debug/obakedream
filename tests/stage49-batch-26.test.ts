...
import {
  batch26MotifIds,
  stage49ExtractionAuditCases,
  stage49RealDreamAuditCases,
  stage49RepresentativeDreams,
} from "./fixtures/stage49";

describe("Stage49 batch-26", () => {
  it("registers all batch-26 motifs", () => {
    const motifIds = motifRegistry.motifs.map((motif) => motif.id);

    expect(motifIds).toEqual(expect.arrayContaining(batch26MotifIds));
    expect(batch26MotifIds).toHaveLength(10);
  });

  it("keeps registry warning gates stable after batch-26 addition", () => {
  ...
  it("keeps batch-26 relations classified outside general and stale buckets", () => {
    const records = buildRelationReviewRecords().filter(
      (record) =>
        batch26MotifIds.includes(record.sourceId as (typeof batch26MotifIds)[number]) ||
        batch26MotifIds.includes(record.targetId as (typeof batch26MotifIds)[number]),
    );
  ...
  it("keeps batch-26 copy non-deterministic (no 絶対/必ず/断言/確実に)", () => {
    const forbiddenPhrases = ["絶対", "必ず", "断言", "確実に"];

    for (const motifId of batch26MotifIds) {
  ...
  it.each(stage49ExtractionAuditCases)("extracts documented Stage49 audit case $id", (auditCase) => {
  ...
  it.each(stage49RealDreamAuditCases)("keeps Stage49 real-dream extraction audit comparable for $id", (auditCase) => {
    const motifIds = extractMotifs(auditCase.text).map((candidate) => candidate.motif.id);
  ...
  it.each(stage49RepresentativeDreams)("keeps Stage49 representative dream behavior for $id", (fixture) => {
